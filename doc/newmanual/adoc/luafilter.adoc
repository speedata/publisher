...
Um RelaxNG-Dateien zu validieren gibt es mehrere Möglichkeiten. Neben einen Variante, direkt im XML Editor das Schema zu validieren (siehe den entsprechenden <<ch-schemavalidierung, Abschnitt im Handbuch>>) gibt es die Möglichkeit,  ein externes Programm dafür zu nutzen.
...
Hierzu muss man sich jedoch ein eigenes RelaxNG-Schema erstellen.
...
Diese Funktion validiert die angegebene XML-Datei mit dem im zweiten Parameter angegebenen RelaxNG (XML-Syntax) Schema.
