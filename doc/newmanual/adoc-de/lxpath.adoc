:ast: *
[appendix]
[[ch-lxpath,XPath- und Layoutfunktionen]]
...
Der Publisher akzeptiert in den Attributen `select` und `test` XPath-Ausdrücke.
...
Der speedata Publisher ignoriert https://de.wikipedia.org/wiki/Namensraum_(XML)[XML-Namensräume] in der Voreinstellung. Das ist in der Regel sehr hilfreich, da Namensräume nicht immer leicht zu handhaben sind.
...
Mit `<Options namespaces="strict" />` stellt man das um, so dass der Namensraum bei <<cmd-record,`<Record>`>> und bei <<cmd-processnode,`<ProcessNode>`>> anzugeben ist:
...
<<cmd-processnode,`<ProcessNode>`>> funktioniert ähnlich:
...
`sd:alternating(<typ>, <text>[,<text>,...])`::
   Bei jedem Aufruf wird das nächste Argument zurückgegeben. Wert des Typs ist beliebig, muss aber eindeutig sein. Beispiel: `sd:alternating("tbl", "Weiß","Grau")` könnte für die Hintergrundfarbe von Tabellen benutzt werden. Beispiel siehe <<ch-wechselnde-zeilenfarben>>.
...
`sd:length(<was>[,<einheit>])`::
  Gibt die Länge von _was_ in Einheiten an (aber ohne die Einheit). Zum Beispiel `sd:length('_bleed','mm')` gibt den Wert `3` zurück, falls der Anschnitt (bleed) auf 3mm gesetzt ist. Diese Funktion initialisiert eine Seite. Die Einheit ist auf 'mm' voreingestellt. *was*  kann eines der Werte von `_bleed`, `_pagewidth` oder `_pageheight` (Seit Version 4.19.10.)
...
`sd:symbol(<Wert>[,<Wert>, ...])`::
   Gibt ein Symbol aus dem aktuellen Font zurück. Beispiel: `sd:symbol(123, 444)` gibt die Zeichen mit den Glyph-Nummern (glyph id) 123 und 444 zurück. Die Glyph-Nummer kann z.B. mit https://opentype.js.org/glyph-inspector.html ermittelt werden (Seit Version 5.1.10.)
...
`sd:visible-pagenumber([<Zahl>])`::
   Liefert die Benutzerdefinierte Seitenzahl für die angegebene echte Seitenzahl zurück. Benutzerdefinierte Seitenzahlen können mit <<cmd-definematter>> erzeugt werden. Ohne Argument wird die aktuelle Seitenzahl benutzt.
...
`distinct-values(<Sequenz>)`::
   Gibt die eindeutigen Werte der Sequenz zurück. Beispiel: `distinct-values((1,2,2,3,3,3))` ergibt die Sequenz `(1,2,3)`. (Seit Version 5.1.26.)
...
`format-number(<Zahl>, <Muster>)`::
   Formatiert die Zahl nach dem Muster. Beispiel: `format-number(12345.67, '#,##0.00')` ergibt die Zeichenkette `12,345.67`. (Seit Version 5.1.25.)
...
`round-half-to-even(<Zahl>,<Zahl>`)`::
   Rundet die angegebene Zahl im ersten Argument auf die Anzahl der Nachkommastellen im zweiten Argument. Bei .5 wird auf die nächste gerade Zahl gerundet (Banker's Rounding). (Seit Version 5.1.25.)

`root(<element>)`::
    Gibt das Wurzelelement des Elements zurück.

`starts-with( <string>, <string>)`::
   Gibt wahr (true) zurück, wenn die erste Zeichenkette mit der zweiten anfängt. Beispiel: `starts-with ( "tattoo", "tat")` ergibt `true`.
...
`tokenize(<Eingabe>,<Regexp>)`::
   Die Rückgabe ist eine Sequenz von Zeichenketten. Die Eingabe wird von links nach rechts gelesen. Sobald eine Stelle gefunden wird, auf die der Reguläre Ausdruck passt, wird die bisherige Eingabe zurückgegeben. Beispiel (aus M. Kays XPath / XSLT-Buch): `tokenize("Go home, Jack!", "\W+")` ergibt die Sequenz `"Go", "home", "Jack", ""`.

`translate(<input>,<from>,<to>)`::
   Ersetzt alle Zeichen in `input`, die in `from` vorkommen durch das entsprechende Zeichen in `to`. Beispiel: `translate('banana','an','uo')` ergibt `buouou`. Wenn `to` kürzer als `from` ist, werden die überzähligen Zeichen in `from` gelöscht. Beispiel: `translate('banana','an','u')` ergibt `buu`. (Seit Version 5.1.26)
