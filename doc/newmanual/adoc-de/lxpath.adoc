...
Der Publisher akzeptiert in den den Attributen `select` und `test` XPath-Ausdrücke.
...
`sd:visible-pagenumber([<Zahl>])`::
   Liefert die Benutzerdefinerte Seitenzahl für die angegebene echte Seitenzahl zurück. Benutzerdefinierte Seitenzahlen können mit <<cmd-definematter>> erzeugt werden. Ohne Argument wird die aktuelle Seitenzahl benutzt.
