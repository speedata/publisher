...
`distinct-values(<Sequenz>)`::
   Gibt die eindeutigen Werte der Sequenz zurück. Beispiel: `distinct-values((1,2,2,3,3,3))` ergibt die Sequenz `(1,2,3)`.
...
`format-number(<Zahl>, <Muster>)`::
   Formatiert die Zahl nach dem Muster. Beispiel: `format-number(12345.67, '#,##0.00')` ergibt die Zeichenkette `12,345.67`.
...
`round-half-to-even(<Zahl>,<Zahl>`)`::
   Rundet die angegebene Zahl im ersten Argument auf die Anzahl der Nachkommastellen im zweiten Argument. Bei .5 wird auf die nächste gerade Zahl gerundet (Banker's Rounding).
