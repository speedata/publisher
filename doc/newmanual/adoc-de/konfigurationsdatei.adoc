...
`mode`::
  Setzt einen Modus für die Verarbeitung. Kann im Layout mit <<ch-lxpath,`sd:mode()`>> abgefragt werden. Mehrere Modi werden durch Komma getrennt angegeben. Siehe <<ch-fortgeschrittenethemen-steuerunglayout>>.
`opencommand`::
...
`resizehandler`::
  Zuordnung von Bildtyp zu externen Konvertern, die die Bildgröße an die gewünschte DPI-Zahl anpassen. Z.B. `resizehandler="jpegimage:(magick %%input%% -resize %%width%%x%%height%%! %%output%%)"`. Siehe auch <<ch-dpioption>>. (Seit Version 5.1.23.)
`runs`::
