...
Die Seitenzahl der aktuellen Seite kann mit `sd:current-page()` ermittelt werden. Diese Funktion gibt afu der ersten Seite die Zahl 1 zurück, auf der zweiten Seite die Zahl 2 und so fort. Mit dem `startpage`-Attribut bei <<cmd-options,`<Options>`>> kann man die Startseitenzahl setzen:
...
Hier (mit <<cmd-clearpage,`<Clearpage>`>>) wird der Abschnitt `frontmatter` für die folgenden Seiten benutzt. Das bewirkt die Ausgabe der sichtbaren Seitenzahl als römische Ziffer in Kleinbuchstaben, da dieser Abschnitt wie folgt definiert wurde:
...
Abschnitte können nur mit dem Befehl <<cmd-clearpage,`<Clearpage>`>> eingeleitet werden.
