...
Die Grundlage der Datenverarbeitung im speedata Publisher bildet das Zusammenspiel von `<<cmd-record,Record>>` und `<<cmd-processnode,ProcessNode>>`.
...
Der Publisher verarbeitet die XML-Daten, indem er für jedes Datenelement den passenden `Record` sucht und die darin enthaltenen Anweisungen ausführt.
Der `Record` für das Wurzelelement der Datendatei wird automatisch aufgerufen, alle weiteren Datenelemente müssen explizit mit `ProcessNode` verarbeitet werden.

.Einfaches Beispiel: Elementname
[source, xml]
-------------------------------------------------------------------------------
...
Dieser Record wird ausgeführt, wenn ein `<product>`-Element verarbeitet wird.
...
.Ein Beispiel für Copy-of in der Praxis ist das Zusammenbauen von XML-Strukturen, mit denen Informationen gespeichert werden können. Ausführlich beschrieben wird dieses Beispiel im Kochbuch (Kapitel 8), dort im Abschnitt Verzeichnisse erstellen (XML-Struktur).
[source, xml]
-------------------------------------------------------------------------------
