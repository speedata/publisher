...
Die Adresse kann mit den Parametern `address` und `port` in der Konfigurationsdatei bzw. auf der Kommandozeile geändert werden, siehe <<ch-konfiguration,den Anhang über die Konfiguration>>.
...
| GET | <<api-method-v0-data,`/v0/data/<id>`>> | Lade die `data.xml` aus dem Publishing-Lauf.
| GET | <<api-method-v0-layout,`/v0/layout/<id>`>> | Lade die `layout.xml` aus dem Publishing-Lauf.
| GET | <<api-method-v0-statusfile,`/v0/statusfile/<id>`>> | Lade die Statusdatei (`publisher.status`) aus dem Publishing-Lauf.
| GET | <<api-method-v0-protocol,`/v0/protocol/<id>`>> | Lade die Protokolldatei (`publisher-protocol.xml`) aus dem Publishing-Lauf.
