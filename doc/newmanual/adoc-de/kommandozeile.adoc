...
`--cache=METHOD`::
   Caching-Strategie für http(s) Bilddateien. Entweder `fast`, dann wird nur geschaut, ob die Bilddatei im Dateisystem vorhanden ist oder `optimal` (Voreinstellung), dann wird bei jedem Zugriff auf das Bild geprüft, ob das Bild aktualisiert werden muss. Bei `none` werden die Bilder immer herunter geladen. Siehe auch <<ch-konfiguration,Konfigurationsdatei>>.
`-c, --config=NAME`::
