...
`--xpath`::
   Setze den XML und XPath Parser auf eine Option aus `luxor` und `lxpath`. Die Voreinstellung ist der ältere 'luxor'. Die neue Variante `lxpath` robuster und hat erweiterte Fähigkeiten, ist aber noch experimentell.
`--xml`::
