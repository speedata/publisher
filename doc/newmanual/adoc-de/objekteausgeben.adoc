...
Seit Version 4.11.3 kann der speedata Publsher Bilder und andere Objekte beschneiden. Das resultierende Objekt ist kleiner als das Original, sofern die Methode `shrink` (Voreinstellung) gewählt ist. Andernfalls (Methode `clip`) ist das resultierende Objekt genau so groß wie das Original, es wird nur ein kleinerer Ausschnitt ausgegeben.

Ein Benutzer hat eine schöne Erklärung der Unterschiede zwischen den Methoden `shrink` und `clip`:

Wenn der _Publisher_ eine Schere hätte, würde `shrink` das Bild selber beschneiden und `clip` würde einen Rahmen (Passepartout) mit der Öffnung der angegebenen Werte schneiden und über das Bild legen.
