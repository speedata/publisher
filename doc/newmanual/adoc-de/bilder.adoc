...
Wenn das Programm inkscape installiert ist, wird das genutzt um Dateien im SVG-Format automatisch nach PDF zu konvertieren (siehe dazu den Abschnitt über <<ch-konfiguration,Konfiguration>> - dort können auch die Kommandozeilenparameter für die Konvertierung gesetzt werden).
...
Bilder können innerhalb eines `<<cmd-paragraph,Paragraph>>`-Elements inline platziert werden, zusammen mit Text oder anderen Bildern.
...
Das Attribut `vertical-align` auf `<<cmd-image,Image>>` steuert, wie Bilder zueinander und zum Text ausgerichtet werden.
...
NOTE: Das Attribut `vertical-align` wirkt nur, wenn Bilder inline in einem Absatz verwendet werden. Bei direkter Platzierung über `<<cmd-placeobject,PlaceObject>>` hat es keine Auswirkung.
