...
Im Falle einer PDF Datei, kann man mit dem Dateinamen die Seite und die gewünschte PDF-Box angeben.
Dazu ist der Dateiname wie folgt aufgebaut: dateiname.pdf:seitenzahl:pdfbox.
...
Der Dateiname kann natürlich auch über die üblichen Wege mit XPath zusammengebaut werden, z.B.: `sd:aspectratio(concat("ocean.pdf:",$pagenum,":cropbox"))`.
