...
`list-fonts`::
  Lists all font files found in the Publisher directories. Together with `--xml` this command allows you to copy and paste the output into the layout rules. See <<ch-fonts>>.
...
`--cache=METHOD`::
   Use cache method. One of `none`, `fast` or `optimal`. Default is `optimal`. See the description in the <<ch-configuration>>.
`-c`, `--config=NAME`::
