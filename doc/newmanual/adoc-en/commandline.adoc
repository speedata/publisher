...
`--loglevel=LVL`::
   Set the log level to one of `debug`, `info`, `warn` and `error`. Messages from this level and above are written to the protocol file.
`--mainlanguage=NAME`::
...
`--xpath`::
   Set the XML and XPath parser to one of `luxor` or `lxpath`. Default is the old 'luxor'. The new `lxpath` is more robust and has more capabilities but is still considered experimental.
`--xml`::
