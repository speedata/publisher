...
`filter`::
   Run the given file as a Lua filter. See the section <<ch-preprocessing>>.
`fontpath`::
...
`resizehandler`::
  Assignment of screen type to external converters that resize images to the desired DPI. For example, `resizehandler="jpegimage:(magick %%input%% -resize %%width%%x%%height%%! %%output%%)"`. See also <<ch-dpioption>>. (Since version 5.1.23.)
`runs`::
