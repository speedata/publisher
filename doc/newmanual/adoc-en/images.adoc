...
In the case of a PDF file, the filename can be used to specify the page and the desired PDF box.
For this purpose, the file name is structured as follows: filename.pdf:pagenumber:pdfbox.

Example: `sd:aspectratio("ocean.pdf:1:cropbox")`
...
The filename can of course also be assembled via the usual ways with XPath, e.g.: `sd:aspectratio(concat("ocean.pdf:",$pagenum,":cropbox"))`.
