...
In the case of a PDF file, the file name can be used to specify page number and desired PDF box too.
...
When variables are used, the XPath function `concat()` is required, e.g.: `sd:aspectratio(concat($filename,':',$pagenum,':mediabox'))`.
