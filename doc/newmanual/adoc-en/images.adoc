...
Images can be placed inline within a `<<cmd-paragraph,Paragraph>>`, alongside text or other images.
...
The `vertical-align` attribute on `<<cmd-image,Image>>` controls how images are aligned relative to each other and the text.
...
NOTE: The `vertical-align` attribute only takes effect when images are used inline within a paragraph. It has no effect when an image is placed directly via `<<cmd-placeobject,PlaceObject>>`.
