...
The software can be downloaded for free (see chapter <<ch-installation,Installation>>) and is started via the console or shell. The command to start the Publisher is called `sp`. With it, all functionality can be used. Parameters can be specified via the command line or a configuration file.
