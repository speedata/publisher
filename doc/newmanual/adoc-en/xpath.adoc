...
This page describes the default XPath parser, called “luxor”. There is a new XPath module called “lxpath”. To switch between these two, you can set the default by setting the xpath configuration, for example on the <<ch-commandline,command line>> with
