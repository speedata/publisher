...
[[ch-pagetypes-pagenumbers]]
== Pagenumbers


The page number of the current page can be determined with `sd:current-page()`. This function returns the number 1 on the first page, the number 2 on the second page and so on. The `startpage` attribute at <<cmd-options,`<options>`>> can be used to set the start page number:
...
Here (with <<cmd-clearpage,`<Clearpage>`>>) the matter `frontmatter` is used for the following pages. This causes the visible page number to be output as a Roman numeral in lower case, as this section has been defined as follows:
...
Matters can only start with the command <<cmd-clearpage,`<Clearpage>`>>.
