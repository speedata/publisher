...
* **Füllpunkte zur Seitenzahl**: Statt der Tabelle kann jeder Eintrag ein Absatz mit Tabulatoren sein, etwa mit `tab-stops="100% right leader(' . ')"` im Textformat und `<Value select="concat(@name, '&#9;', @page)"/>`. Die Punkte führen dann vom Titel zur rechtsbündigen Seitenzahl, siehe [Tabulatoren]({{< relref "/manual/text/tabstops" >}}).
