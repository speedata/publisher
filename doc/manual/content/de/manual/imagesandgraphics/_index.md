...
Das Bild  `_samplea.pdf` ist (wie `_sampleb.pdf`) Bestandteil des Publishers und für Testzwecke gut nutzbar.
...
Wenn das Programm inkscape installiert ist, wird das genutzt um Dateien im SVG-Format automatisch nach PDF zu konvertieren (siehe dazu den Abschnitt über [Konfiguration]({{< relref "configuration" >}}) - dort können auch die Kommandozeilenparameter für die Konvertierung gesetzt werden).
...
Daher kann es sich lohnen, für die Verarbeitung spezielle Versionen mit kleineren Dateigrößen bereitzuhalten.
...
```
{{% codecaption %}}Das Bild wird horizontal gestreckt.{{% /codecaption %}}

![Ist clip auf 'no' gesetzt, wird das Bild verzerrt.](/img/18-clip-no.png)
...
![Ist clip auf 'yes' gesetzt, wird nur ein Ausschnitt gezeigt.](/img/18-clip-yes.png)
...
Um die natürliche Größe zu benutzen, aber Einschränkungen anzugeben, gibt es die vier Kombinationen aus min/max und width/height. Das Bild in dem folgenden Beispiel wird nicht breiter als 10 Rasterzellen und nicht höher als 3. Das Seitenverhältnis bleibt erhalten:
...
![Das Bild ist auf die Höhe von drei Rasterzellen beschränkt.](/img/18-maxheight.png)
...
Beispielsweise können die Bilder in dem Unterverzeichnis `images` liegen.
Siehe dazu den Abschnitt über [Dateiorganisation]({{< relref "fileorganization" >}}).
...
Bei jedem Request wird überprüft, ob das Bild noch aktuell ist und ggf. nicht erneut heruntergeladen.
Ist das Bild auf dem Server gelöscht, wird es auch im lokalen System gelöscht.

Mit `sp --cache=fast` kann man auf die schnelle Caching-Methode zurückgreifen, die das Bild nur einmal vom Server herunterlädt und dann nicht wieder auf Aktualität prüft.
...
![Dass die Bilddatei nicht gefunden wurde, sollte sofort zu erkennen sein.](/img/18-fnf.png)
...
Man kann auch mehrere Handler für verschiedene Dateitypen angeben:
...
Zu MetaPost gibt es ein [eigenes Kapitel]({{< relref "metapostgraphics" >}}). Hier ist jedoch gezeigt, mit welchen Möglichkeiten diese Grafiken eingebunden werden können:
...
Die Definition funktioniert analog zu einem `imagehandler`, jedoch stehen die Platzhalter `%%width%%` und `%%height%%` für die automatisch ermittelte Zielgröße zur Verfügung. Der Benutzer gibt diese Werte nicht selbst an – sie werden vom Publisher anhand der Ausgabegröße und der eingestellten Auflösung (`dpi`) berechnet.
...
In diesem Beispiel wird für JPEG-Dateien der Befehl `magick` (ImageMagick) aufgerufen, um das Bild intern auf die vom Publisher berechnete Größe herunterzuskalieren. Das Ausrufezeichen (`!`) erzwingt die exakte Zielgröße.
Diese Mechanik stellt sicher, dass die Ausgabegröße und die gewünschte Auflösung eingehalten werden, ohne dass der Benutzer manuell eingreifen muss.

Wichtig ist, dass auch ein extensionhandler für die Dateiendung (im Beispiel `jpg` oder `jpeg`) gesetzt ist, damit der resizehandler auch genutzt wird.
...
Jedes Bild steht auf der Textgrundlinie. Höhere Bilder ragen weiter nach oben, niedrigere weniger weit. Die Oberkanten sind nicht ausgerichtet.
...
Alle Oberkanten liegen auf einer horizontalen Linie. Die Grundlinie verschiebt sich an die Unterkante des höchsten Bildes, sodass Text unten erscheint.
...
Die vertikalen Mitten aller Bilder sind ausgerichtet. Die Grundlinie liegt auf halber Höhe des höchsten Bildes.
...
Jedes Bild hängt unterhalb der Textgrundlinie. Die Oberkante jedes Bildes liegt auf der Grundlinie und das Bild ragt nach unten. Das ist nützlich, wenn Text über einer Bildreihe stehen soll.
