...
Die letzten vier werden in Tabellen benutzt. Siehe den Abschnitt [Textformate in Tabellen]({{< relref "/manual/tables#textformate-in-tabellen" >}}).
...
Die bekannten CSS-Farben sind im RGB-Farbraum definiert. Die Farben `black` und `white` sind im Graustufen-Farbraum definiert. Siehe auch den Befehl [`DefineColor`]({{< relref "/reference/commands/definecolor" >}}), dort sind die vordefinierten Farben aufgelistet.
