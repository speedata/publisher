...
Mit `<Options namespaces="strict" />` stellt man das um, so dass der Namensraum bei [`<Record>`]({{< relref "/reference/commands/record" >}}) und bei [`<ProcessNode>`]({{< relref "/reference/commands/processnode" >}}) anzugeben ist:
...
[`<ProcessNode>`]({{< relref "/reference/commands/processnode" >}}) funktioniert ähnlich:
