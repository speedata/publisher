...
![Various text markups](/img/textauszeichnungen.png)
...
![If `fontexpansion="some"` is switched on, the wrapped paragraph differs only slightly. On the right, both paragraphs are superimposed, the paragraph for which `fontexpansion` is set to `some` is green.](/img/fontexpansionsome.png)

![With `fontexpansion="yes"` you have to experiment a little to determine a good degree of stretching and compression so that the paragraphs also look "good". On the left, `fontexpansion` is set to `no`, for the other two to `yes`. The paragraph on the right is set with the values 5% stretch and 3% compress (50 and 30 for `fontstretch` and `fontshrink`).](/img/fontexpansionyes.png)
...
![Left: Comma, period and hyphen protrude into the right margin. Right: without optical margin alignment.](/img/marginprotrusion.png)
...
![The default setting for NoBreak is that the part must be connected.](/img/nobreak1.png)
...
![Here the text is cut off and filled with the specified text.](/img/nobreak2.png)
