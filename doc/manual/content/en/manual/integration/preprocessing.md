---
title: "Lua-Filter / pre-processing"
weight: 63
type: docs
---
...
The specified Lua script will be executed before the PDF file generation starts. It must be within [the publisher search path]({{< relref "fileorganization" >}}).
...
Instructions are available at https://relaxng.org/tutorial-20011203.html.
...
See also the example in [examples repository](https://github.com/speedata/examples/tree/master/technical/jsonreader).
...
The second parameter at `csv.decode()` is optional.
...
The example repository shows how to [create an XML file from the CSV file](https://github.com/speedata/examples/tree/master/technical/csvreader).
...
Table with configuration values (see [how to configure the speedata Publisher]({{< relref "configuration" >}})). Can be used for reading and writing the settings.
...
`charset`:
If the CSV file is Latin-1 encoded, this value must be ISO-8859-1. Other encodings on request. The default (without the charset parameter) is UTF-8.
`separator`:
