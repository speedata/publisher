...
![Automatic flow around objects that were previously output.](/img/umfliessenvonbildern.png)
...
The file `pocketwatch.pdf` can have an outline file with the same name and the extension xml, which is structured as follows
...
```
{{% codecaption %}}The segments determine the occupied area. The specifications refer to the (arbitrary) unit of 75x100 units.{{% /codecaption %}}

![The shape of the watch must be determined in a preparatory step.](/img/taschenuhr.png)

{{< callout >}}
This functionality is still experimental. The next versions of the Publisher will probably have improvements to this feature. The outline of an image can be created with the program imageshaper at https://github.com/speedata/imageshaper.
{{< /callout >}}
