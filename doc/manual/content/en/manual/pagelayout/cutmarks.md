...
: Bleed marks and bleeding can be controlled separately (see below or the command [`<Options>`]({{% relref "/reference/options" %}}))
...
: The Publisher can issue a warning if this is below a certain value (attribute “dpiwarn” see [`<Image>`]({{% relref "/reference/image" %}})).
...
To switch on bleed or crop marks, these must be activated in the element [`<Options>`]({{< relref "/reference/options" >}}):
...
{{< callout >}}
The command [`<PDFOptions>`]({{< relref "/reference/pdfoptions" >}}) offers the possibility to set preferences for the print dialog in the PDF viewer.
{{< /callout >}}
