...
![Left: thumb indexes in the phone book (that still exists!). Right: Lines are less obtrusive, yet practical.](/img/25-griffmarken.jpg)
...
In the example, the chapter number is stored in the variable `chapter` and output in a loop.
In practice, this would of course be inserted into the page type (`AtPageShipout`) and only one thumb index marker would be output at a time.

![The green line shows the final format, the blue line the outer edge of the untrimmed paper.](/img/25-publisher.png)
