...
`--autoopen`
: Open the PDF file. Can be set in the [How to configure the speedata publisher]({{% relref "configuration" %}}).

`--cache=METHOD`
: Use cache method. One of `none`, `fast` or `optimal`. Default is `optimal`. See the description in the [How to configure the speedata publisher]({{% relref "configuration" %}}).
...
`--mode=NAME`
: Set mode. Multiple modes given in a comma separated list. See [advanced-controllayout]({{% relref "controllayout" %}}).
