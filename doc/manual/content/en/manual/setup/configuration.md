...
`addlocalpath`
: If true, the publisher adds the current working directory to the path. Default: true.
...
`mode`
: Set the layout mode. See [advanced-controllayout]({{% relref "controllayout" %}}).
...
`resizehandler`
: Assignment of screen type to external converters that resize images to the desired DPI. For example, `resizehandler="jpegimage:(magick %%input%% -resize %%width%%x%%height%%! %%output%%)"`. See also [dpioption]({{% relref "configuration" %}}). (Since version 5.1.23.)
...
`xpath`
: [Set the XML module]({{% relref "xpath" %}}). The current default is `lxpath`, and the old is called `luxor`.
...
The XML layout file has a command called [`<Options>`]({{< relref "/reference/options" >}}) that allows to set some parameters (tracing, default language, …)
