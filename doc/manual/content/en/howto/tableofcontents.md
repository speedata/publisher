...
* **Dot leaders to the page number**: Instead of the table, each entry can be a paragraph with tabs, for example with `tab-stops="100% right leader(' . ')"` in the text format and `<Value select="concat(@name, '&#9;', @page)"/>`. The dots then lead from the title to the page number flush right, see [Tab stops]({{< relref "/manual/text/tabstops" >}}).
