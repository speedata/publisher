...
In addition, there are [XPath standard functions]({{< relref "xpathfunctions" >}}) and [user-defined functions]({{< relref "/reference/commands/function" >}}).
...
`sd:pagenumber(<mark>)`
: Return the page number where the given mark was placed. See [Mark]({{% relref "/reference/commands/mark" %}}).
