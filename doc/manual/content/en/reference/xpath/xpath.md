...
With `<Options namespaces="strict" />` you can change this so that the namespace must be specified for [`<Record>`]({{< relref "/reference/commands/record" >}}) and [`<ProcessNode>`]({{< relref "/reference/commands/processnode" >}}):
...
[`<ProcessNode>`]({{< relref "/reference/commands/processnode" >}}) works similar:
