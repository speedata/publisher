...
* Wildcard (@*@) als "XPath-Ausdruck":xpath (1.5.32)
* "BearbeiteKnoten":e_bearbeiteknoten erlaubt "XPath-Ausdrücke":xpath (1.5.32)
* "QR Codes":e_strichcode (1.5.28)
* Neues Element "FürAlle":e_fueralle, um für alle Kindelemente Befehle auszuführen. (1.5.27)
* sp based hotfolder (@sp watch@) (1.5.26)
* "CSS Stylesheets":e_stylesheet
* "Barcodes (EAN13 und Code128)":e_strichcode
* XPath-Funktion "count()":xpath
* Linienstärke bei "ObjektAusgeben":e_objektausgeben
* CDATA Abschnitte im lpeg-XML-Parser sind erlaubt
* Neue XPath-Funktion "sd:aktuelle-spalte()":xpath 
* @sp@ liest Daten von STDIN wenn der Datenname @-@ ist.
* "Td":e_td Attribut @align@ darf (wie im Handbuch beschrieben) nur noch 'left', 'right' oder 'center' als Werte enthalten.
* "Tabellenfuß":e_tabellenfuss und "Tabellenkopf":e_tabellenkopf erlauben eine Seitenangabe
* @sp@: @--[no-]local@ um das lokale Verzeichnis nicht zu beachten
* @sp@: @--verbose@ erzeugt zusätzliche Informationen
* Neue XPath-Funktion "sd:formatiere-zahl()":xpath
...
* Keine Default-Sprache mehr, wird nun in "Optionen":e_optionen festgelegt.
...
* Default Textformat bei "Tabellen":e_tabelle.
* @zeilen@-Angabe bei Textformaten (inkl hängender Einzug). Siehe "DefiniereTextformat":e_definieretextformat
* @sprun@ durch @sp@ ersetzt. Siehe "Kommandozeile":kommandozeile
* @hreferenz@ bei "ObjektAusgeben":e_objektausgeben.
* Neue XPath-Funktion "empty()":xpath
* Zeilenangabe in "NeueZeile":e_neuezeile.
* Linienfarbe veränderbar. "Linie":e_linie.
* Bildumrisse können angegeben werden. Siehe "Bild":e_bild und "ObjektAusgeben":e_objektausgeben
* @--laeufe=...@-Option bei @sprun@
* @Zeilennummer@ in "NeueZeile":e_neuezeile
...
* @-v@, @--variable@ in "sprun":kommandozeile
* @valign@ in "Td":e_td, "Tr":e_tr: @middle@ statt @center@
* @valign@ in "ObjektAusgeben":e_objektAusgeben
* PDF "Lesezeichen":e_lesezeichen.
