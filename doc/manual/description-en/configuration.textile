...
- @extra_dir@ := Ein Verzeichnis im Dateisystem, das Dateien für den Publisherlauf enthält. Dazu gehören die Schriftdateien, die XML-Dateien (Daten und Layoutregelwerk) sowie die einzubindenden Bilddateien. Kann mehrfach angegeben werden. Das aktuelle Verzeichnis wird automatisch eingebunden.
...
Alle Angaben in dieser Konfigurationsdatei sind optional. Alle folgenden Konfigurationsdateienwerden nin dieser Reihenfolge eingelesen: @/etc/speedata/publisher.cfg@, @~/.publisher.cfg@ und im aktuellen Verzeichnis @publisher.cfg@.
...
Das Layoutregelwerk erlaubt manche Parameter zu setzen. Dazu gehören die Angaben, die im Element "Optionen":../commands-en/options.html gesetzt werden.
