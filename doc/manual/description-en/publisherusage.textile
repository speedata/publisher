...
This is what the terminal window on Linux looks like. On other systems it should be similar:
...
The speedata Publisher expects the data XML file with the name @data.xml@ and the layout XML with the name @layout.xml@ (this can be changed on the command and set in the configuration file) in the current directory or any of its subdirectories. This search path can be extended by configuring the publisher (see the section on "how to configure the speedata publisher":configuration.html).
