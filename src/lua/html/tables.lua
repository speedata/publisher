...
function M.build_html_table_tbody(tbody, cb, dataxml)
    local trtab = {}
        ...
        if type(tr) == "table" and tr.elementname then
            ...
            for cell = 1, #tr do
                ...
                if type(td) == "table" and td.elementname then
                    ...
                    local r = cb.build_nodelist(td, {}, nil, "build_html_table_tbody/td", nil, dataxml)
...
function M.build_html_table(table_elt, width, cb, dataxml)
    ...
    for i = 1, #tablecontents do
        ...
        if typ == "table" then
            ...
            if eltname == "tbody" then
                body = M.build_html_table_tbody(thiselt, cb, dataxml)
            elseif eltname == "tfoot" then
                foot = M.build_html_table_tbody(thiselt, cb, dataxml)
            elseif eltname == "thead" then
                head = M.build_html_table_tbody(thiselt, cb, dataxml)
            end
        end
    end
