...
function M.define_font_hb(name, size, extra_parameter)
    ...
    -- With spacefromfont="yes" at LoadFontfile the interword glue comes from
    ...
    -- explicit space attribute keeps the old calculation, as do fonts
    -- without a space glyph (some emoji fonts). The flag on the font table
    -- lets the paragraph builder treat preserved spaces the same way.
    if extra_parameter.spacefromfont and not extra_parameter.space then
