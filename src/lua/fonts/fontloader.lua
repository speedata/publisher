...
function M.define_font(name, size, extra_parameter)
    ...
    if not fonttable.units_per_em then
        return false, string.format("something went wrong defining %s. Please file a bug report.", name)
    end
    ...
    if f.format == nil then
        return false, "Could not determine the type of the font '" .. fonttable.filename_with_path .. "'."
    end
    ...
    for i = 1, #fonttable.glyphtable do
        ...
        if glyph.name:lower():match("^hyphen$") then
            codepoint = 45
        end
        if codepoint then
            ...
            if glyph.boundingbox[4] then
                f.characters[codepoint].height = glyph.boundingbox[4] * mag
            end
            if glyph.boundingbox[2] then
            ...
            if
                ...
                and extra_parameter
                and type(extra_parameter.marginprotrusion) == "number"
            then
