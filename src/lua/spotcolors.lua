...
function set_colorprofile(name)
    ...
    if not tmp then
        main.log(
            "error",
            string.format("Color profile with ID %q not found. Using default %q", tostring(name), "FOGRA39")
        )
...
function write_colorprofile()
    if currentcolorprofile.objectid == nil or currentcolorprofile.objectid == 0 then
        ...
        if not path then
            ...
            currentcolorprofile.objectid =
                pdf.immediateobj("streamfile", path, string.format("/N %d", currentcolorprofile.colors or 4))
        end
    end
    ...
    if not cmyktable then
        if c == nil or m == nil or y == nil or k == nil then
            main.log(
                "warn",
                string.format(
                    "You must define a CMYK substitute for the spot color %q in order to display it in the PDF.",
                    rawname
                )
            )
...
function use_color(colorname)
    ...
    if cmyktable.usecolorprofile then
        ...
        separationobjnum = string.format(
            [==[[/Separation %s [ /ICCBased %d 0 R ]  << /FunctionType 2 /C0 [ 0 0 0 0 ] /C1 [ %g %g %g %g ] /Domain [ 0 1 ] /N 1 >> ]]==],
            pdfcolorname,
            cp,
            c / 100,
            m / 100,
        ...
        separationobjnum = string.format(
            [==[[/Separation %s /DeviceCMYK << /FunctionType 2 /C0 [ 0 0 0 0 ] /C1 [ %g %g %g %g ] /Domain [ 0 1 ] /N 1 >> ]]==],
            pdfcolorname,
            c / 100,
            m / 100,
            y / 100,
