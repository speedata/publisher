...
func FillBuildDir(cfg *config.Config, luatexbindir, bindir, sharedir, swdir string) error {
	...
	mapping := []struct {
		...
		{src: filepath.Join(cfg.Basedir(), "lib"), dest: filepath.Join(sharedir, "lib"), reject: []string{".gitignore", "libsplib.h", "libsplib.dll", "libsplib.so", "libsplib.dylib", "rlib.so", "rlib.dll", "trang.jar", "luaglue.so"}},
